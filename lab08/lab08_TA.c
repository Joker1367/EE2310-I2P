...
int main(void)
{
	long int N = 10000000;                      // amout of loop
	...
	//	 else return 0
{
	...
	while (i <= 8) {
		...
		if (find == 0 && i > 8)                  // if not find, return 0
			return 0;
		else if (find) {                         // if find return 1, end loop
			i = 9;
			return 1;
		}
		else return 0;
	...
	//	 else return 0
{
	...
	for (i = 0; i < 4 && find; i++) {      
		if (col[i] == 5) {                       // if exist flush return 1
			find = 0;                            // if find, change find to 0
			return 1;
		}
		else;
	}
	if (find)                                    // if not find, return 0      
		return 0;
	else return 0;
}

int Common (int num[13])
	// To check for other coditions except flush and straight
	//	 if there's a Four of a kind, return 1
	//	 if there's a Full housem return 2
